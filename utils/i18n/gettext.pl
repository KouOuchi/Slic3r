...
if(-f "$PO_DIRECTORY/$PO_NAME.pot") {
    # exists pot
    if ($^O eq 'MSWin32') {
	system('cmd.exe', '/K', "$^X utils/i18n/remove-potcdata.pl < $PO_DIRECTORY/$PO_NAME.pot > $PO_DIRECTORY/$PO_NAME.1po") == 0
	    or die $@;
	system('cmd.exe', '/K', "$^X utils/i18n/remove-potcdata.pl < $PO_DIRECTORY/$PO_NAME.po > $PO_DIRECTORY/$PO_NAME.2po") == 0
	    or die $@;
    } else {

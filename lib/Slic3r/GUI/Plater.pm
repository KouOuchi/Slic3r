...
use constant CANVAS_TEXT => join('-', +(localtime)[3,4]) eq '13-8'
    ? Slic3r::_u('What do you want to print today?') . '™' # Sept. 13, 2006. The first part ever printed by a RepRap to make another RepRap.
    : Slic3r::_u('Drag your objects here');
...
sub new {
    ...
    if (!&Wx::wxMSW) {
        ...
        $self->{htoolbar}->AddTool(TB_LOAD, Slic3r::_u("Add") . "…", Wx::Bitmap->new("$Slic3r::var/brick_add.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddTool(TB_REMOVE, Slic3r::_u("Delete"), Wx::Bitmap->new("$Slic3r::var/brick_delete.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddTool(TB_RESET, Slic3r::_u("Delete All"), Wx::Bitmap->new("$Slic3r::var/cross.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddTool(TB_ARRANGE, Slic3r::_u("Arrange"), Wx::Bitmap->new("$Slic3r::var/bricks.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddSeparator;
        $self->{htoolbar}->AddTool(TB_MORE, Slic3r::_u("More"), Wx::Bitmap->new("$Slic3r::var/add.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddTool(TB_FEWER, Slic3r::_u("Fewer"), Wx::Bitmap->new("$Slic3r::var/delete.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddSeparator;
        $self->{htoolbar}->AddTool(TB_45CCW, "45° ccw", Wx::Bitmap->new("$Slic3r::var/arrow_rotate_anticlockwise.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddTool(TB_45CW, "45° cw", Wx::Bitmap->new("$Slic3r::var/arrow_rotate_clockwise.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddTool(TB_ROTATE, Slic3r::_u("Rotate") . "…", Wx::Bitmap->new("$Slic3r::var/arrow_rotate_clockwise.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddTool(TB_SCALE, Slic3r::_u("Scale") . "…", Wx::Bitmap->new("$Slic3r::var/arrow_out.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddTool(TB_SPLIT, Slic3r::_u("Split"), Wx::Bitmap->new("$Slic3r::var/shape_ungroup.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddSeparator;
        $self->{htoolbar}->AddTool(TB_VIEW, Slic3r::_u("View"), Wx::Bitmap->new("$Slic3r::var/package.png", wxBITMAP_TYPE_PNG), '');
        $self->{htoolbar}->AddTool(TB_SETTINGS, Slic3r::_u("Settings") . "…", Wx::Bitmap->new("$Slic3r::var/cog.png", wxBITMAP_TYPE_PNG), '');
    } else {
        my %tbar_buttons = (
            load            => "Add…",
            remove          => "Delete",
    ...
    # right pane buttons
    $self->{btn_export_gcode} = Wx::Button->new($self, -1, Slic3r::_u("Export G-code") . "…", wxDefaultPosition, wxDefaultSize, wxBU_LEFT);
    $self->{btn_export_stl} = Wx::Button->new($self, -1, Slic3r::_u("Export STL") . "…", wxDefaultPosition, wxDefaultSize, wxBU_LEFT);
...

    {
        ...
        {
            my $box = Wx::StaticBox->new($self, -1, Slic3r::_u("Info"));
            ...
            my @info = (
                size        => Slic3r::_u("Size"),
                volume      => Slic3r::_u("Volume"),
                facets      => Slic3r::_u("Facets"),
                materials   => Slic3r::_u("Materials"),
                manifold    => Slic3r::_u("Manifold"),
            );
